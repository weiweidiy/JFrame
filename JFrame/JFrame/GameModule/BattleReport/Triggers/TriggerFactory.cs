...
namespace JFrame
{
    ...
    public class NewTriggerFactory
    {
        public INewBattleTrigger Create(PVPBattleManager pvpBattleManager, int triggerType, float arg, float delay = 0)
        {
            switch (triggerType)
            {
                case 1: //无
                ...
                case 2: //自身死亡触发
                    return new NewDeathTrigger(pvpBattleManager, new float[1] { arg }, delay);
                case 3: //战斗开始触发
                    return new NewBattleStartTrigger(pvpBattleManager, new float[1] { arg }, delay);
                case 4: //己方有非满血成员
                    return new NewFriendsHurtTrigger(pvpBattleManager, new float[1] { arg }, delay);
                case 5: //指定action释放
                    return new NewActionCastTrigger(pvpBattleManager, new float[1] { arg }, delay);
                default:
                    throw new Exception(triggerType + " 技能未实现的 ConditionTrigger type " + triggerType);
            }
        }
